def so2_create_NC_file_v2(dout, DT):
   from netCDF4 import Dataset
   
   f1 = 'ncas-43i-so2-1' #instrument name
   ...
   f4 = 'so2-concentration' #data product
...
def so2_NC_Global_Attributes_v2(fn_nc, meta, ET):
...
def so2_NC_Dimensions_v2(fn_nc, ET):
...
def so2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon):
   #time
   ...
   #SO2 conc
   SO2 = fn_nc.createVariable('mole_fraction_of_sulfur_dioxide_in_air', np.float32, ('time',),fill_value=-1.00e+20)
   #variable attributes
   SO2.type = 'float32'
   SO2.units = '1e-9'
   SO2.practical_units = 'ppb'
   SO2.standard_name = 'mole_fraction_of_sulfur_dioxide_in_air'
   SO2.long_name = 'Mole Fraction of Sulfur Dioxide in air'
   XX = data.so2
   np.putmask(XX, data.flag != 1, np.nan)
   SO2.valid_min = np.float32(np.nanmin(XX))
   SO2.valid_max = np.float32(np.nanmax(XX))
   SO2.cell_methods = 'time: point'
   SO2.coordinates = 'latitude longitude'
   SO2.chemical_species = 'SO2'
   #write data
   SO2[:] = np.float32(data.so2)
...
def NC_so2_v2(pd, np, dout, meta, data):
   fn_nc = so2_create_NC_file_v2(dout, data.DT)
   
   lat, lon = so2_NC_Global_Attributes_v2(fn_nc, meta, data.ET)
   so2_NC_Dimensions_v2(fn_nc, data.ET)
   so2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon)
