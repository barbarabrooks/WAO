def o2_create_NC_file_v2(dout, DT):
   from netCDF4 import Dataset
   
   f1 = 'ncas-o2-1' #instrument name
   ...
   f4 = 'o2n2-concentration-ratio' #data product
...
def o2_NC_Global_Attributes_v2(fn_nc, meta, ET):
...
def o2_NC_Dimensions_v2(fn_nc, ET):
...
def o2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon):
   #time
   ...
   #o2 conc
   O2 = fn_nc.createVariable('molecular_oxygen_molecular_nitrogen_ratio_in_air', np.float32, ('time',),fill_value=-1.00e+20)
   #variable attributes
   O2.type = 'float32'
   O2.units = 'per meg'
   O2.long_name = 'O2/N2 ratio in air'
   XX = data.o2n2
   np.putmask(XX, data.flag != 1, np.nan)
   O2.valid_min = np.float32(np.nanmin(XX))
   O2.valid_max = np.float32(np.nanmax(XX))
   O2.cell_methods = 'time: point'
   O2.coordinates = 'latitude longitude'
   #write data
   O2[:] = np.float32(data.o2n2)
   ...
   qc_flags.flag_meanings = qc_flags.flag_meanings + 'suspect_data_gas_concentration_outside_instrument_operational_range' + '\n'
   qc_flags.flag_meanings = qc_flags.flag_meanings + 'suspect_data_time_stamp_error' 
   #write data
   qc_flags[:] = np.int8(data.flag)
   
def NC_o2_v2(pd, np, dout, meta, data):
   fn_nc = o2_create_NC_file_v2(dout, data.DT)
   
   lat, lon = o2_NC_Global_Attributes_v2(fn_nc, meta, data.ET)
   o2_NC_Dimensions_v2(fn_nc, data.ET)
   o2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon)
