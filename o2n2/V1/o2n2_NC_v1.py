...
def wao_o2_create_NC_file_v1(meta, dout, DT):
...
def wao_o2_NC_Global_Attributes_v1(nc, meta, ET):
...
def wao_o2_NC_VaraiblesAndData_v1(nc, meta, data, np):
   ...
   #o2 conc
   O2 = nc.createVariable('o2n2_concentration_ratio_in_air', np.double, ('time',),fill_value=-1.00e+20)
   #bks variable attributes
   O2.dimension = 'time'
   O2.type = 'double'
   O2.units = 'per meg'
   O2.long_name = 'O2:N2 concentration ratio in air'
   O2.valid_min = -900
   O2.valid_max = -200
...
def NC_wao_o2_v1(meta, dout, data, np):
   nc = wao_o2_create_NC_file_v1(meta, dout, data.DT)
   
   wao_o2_NC_Global_Attributes_v1(nc, meta, data.ET)
   wao_o2_NC_Dimensions_v1(nc, data.ET)
   wao_o2_NC_VaraiblesAndData_v1(nc, meta, data, np)
