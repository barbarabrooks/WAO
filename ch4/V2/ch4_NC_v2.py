def ch4_create_NC_file_v2(dout, DT):
   from netCDF4 import Dataset
   
   f1 = 'ncas-ghg-gc-fid-1' #instrument name
   ...
   f4 = 'ch4-concentration' #data product
...
def ch4_NC_Global_Attributes_v2(fn_nc, meta, ET):
...
def ch4_NC_Dimensions_v2(fn_nc, ET):
...
def ch4_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon):
   #time
...
def NC_ch4_v2(pd, np, dout, meta, data):
   fn_nc = ch4_create_NC_file_v2(dout, data.DT)
   
   lat, lon = ch4_NC_Global_Attributes_v2(fn_nc, meta, data.ET)
   ch4_NC_Dimensions_v2(fn_nc, data.ET)
   ch4_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon)
