def n2osf6_create_NC_file_v2(dout, DT):
   from netCDF4 import Dataset
   
   f1 = 'ncas-ghg-gc-ecd-1' #instrument name
   ...
   f4 = 'n2o-sf6-concentration' #data product
...
def n2osf6_NC_Global_Attributes_v2(fn_nc, meta, ET):
...
def n2osf6_NC_Dimensions_v2(fn_nc, ET):
...
def n2osf6_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon):
   #time
   ...
   qc_flags_n2o.flag_meanings = qc_flags_n2o.flag_meanings + 'suspect_data_unspecified_instrument_performance_issues_contact_data_originator_for_more_information' + '\n'
   qc_flags_n2o.flag_meanings = qc_flags_n2o.flag_meanings + 'suspect_data_time_stamp_error' 
   #write data
   qc_flags_n2o[:] = np.int8(data.flag_n2o)
   
   #SF6 conc
   SF6 = fn_nc.createVariable('mole_fraction_of_sulfur_hexafluoride_in_air', np.float32, ('time',),fill_value=-1.00e+20)
   #variable attributes
   SF6.type = 'float32'
   SF6.units = '1e-12'
   SF6.practical_units = 'ppt'
   SF6.long_name = 'Mole Fraction of Sulfur Hexafluoride in dry air'
   XX = data.sf6
   np.putmask(XX, data.flag_sf6 != 1, np.nan)
   SF6.valid_min = np.float32(np.nanmin(XX))
   SF6.valid_max = np.float32(np.nanmax(XX))
   SF6.cell_methods = 'time: point'
   SF6.coordinates = 'latitude longitude'
   SF6.chemical_species = 'SF6'
   #write data
   SF6[:] = np.float32(data.sf6)
   
   #Qc flag SF6
   qc_flags_sf6 = fn_nc.createVariable('qc_flag_sf6', np.int8, ('time',))
   #variable attribute
   qc_flags_sf6.type = 'byte'
   qc_flags_sf6.units = '1'
   qc_flags_sf6.long_name = 'Data Quality Flag: SF6'
   qc_flags_sf6.flag_values = '0b,1b,2b,3b'
   qc_flags_sf6.flag_meanings = 'not_used' + '\n'
   qc_flags_sf6.flag_meanings = qc_flags_sf6.flag_meanings + 'good_data' + '\n'
   qc_flags_sf6.flag_meanings = qc_flags_sf6.flag_meanings + 'suspect_data_unspecified_instrument_performance_issues_contact_data_originator_for_more_information' + '\n'
   qc_flags_sf6.flag_meanings = qc_flags_sf6.flag_meanings + 'suspect_data_time_stamp_error' 
   #write data
   qc_flags_sf6[:] = np.int8(data.flag_sf6)
   
def NC_n2osf6_v2(pd, np, dout, meta, data):
   fn_nc = n2osf6_create_NC_file_v2(dout, data.DT)
   
   lat, lon = n2osf6_NC_Global_Attributes_v2(fn_nc, meta, data.ET)
   n2osf6_NC_Dimensions_v2(fn_nc, data.ET)
   n2osf6_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon)
