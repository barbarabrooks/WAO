def no2_create_NC_file_v2(dout, DT):
   from netCDF4 import Dataset
   
   f1 = 'uea-caps-1' #instrument name
   ...
   f4 = 'no2-concentration' #data product
...
def no2_NC_Global_Attributes_v2(fn_nc, meta, ET):
...
def no2_NC_Dimensions_v2(fn_nc, ET):
...
def no2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon):
   #time
   ...
   #NO2 conc
   NO2 = fn_nc.createVariable('mole_fraction_of_nirogen_dioxide_in_air', np.float32, ('time',),fill_value=-1.00e+20)
   #variable attributes
   NO2.type = 'float32'
   NO2.units = '1e-9'
   NO2.practical_units = 'ppb'
   NO2.standard_name = 'mole_fraction_of_nirogen_dioxide_in_air'
   ...
   qc_flags.flag_meanings = qc_flags.flag_meanings + 'bad_data_gas_concentration_outside_instrument_operational _range' + '\n'
   qc_flags.flag_meanings = qc_flags.flag_meanings + 'suspect_data_time_stamp _error' 
   #write data
   qc_flags[:] = np.int8(data.flag)
   
def NC_no2_v2(pd, np, dout, meta, data):
   fn_nc = no2_create_NC_file_v2(dout, data.DT)
   
   lat, lon = no2_NC_Global_Attributes_v2(fn_nc, meta, data.ET)
   no2_NC_Dimensions_v2(fn_nc, data.ET)
   no2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon)
