def co2_create_NC_file_v2(dout, DT):
   from netCDF4 import Dataset
   
   f1 = 'ncas-co2-1' #instrument name
   ...
   f4 = 'co2-concentration' #data product
...
def co2_NC_Global_Attributes_v2(fn_nc, meta, ET):
...
def co2_NC_Dimensions_v2(fn_nc, ET):
...
def co2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon):
   #time
   ...
   #CO2 conc
   CO2 = fn_nc.createVariable('mole_fraction_of_carbon_dioxide_in_air', np.float32, ('time',),fill_value=-1.00e+20)
   #variable attributes
   CO2.type = 'float32'
   CO2.units = '1e-6'
   CO2.practical_units = 'ppm'
   CO2.standard_name = 'mole_fraction_of_carbon_dioxide_in_air'
   CO2.long_name = 'Mole Fraction of Carbon Dioxide in air'
   XX = data.co2
   np.putmask(XX, data.flag != 1, np.nan)
   CO2.valid_min = np.float32(np.nanmin(XX))
   CO2.valid_max = np.float32(np.nanmax(XX))
   CO2.cell_methods = 'time: point'
   CO2.coordinates = 'latitude longitude'
   CO2.chemical_species = 'CO2'
   #write data
   CO2[:] = np.float32(data.co2)
...
def NC_co2_v2(pd, np, dout, meta, data):
   fn_nc = co2_create_NC_file_v2(dout, data.DT)
   
   lat, lon = co2_NC_Global_Attributes_v2(fn_nc, meta, data.ET)
   co2_NC_Dimensions_v2(fn_nc, data.ET)
   co2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon)
