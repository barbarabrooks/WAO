def coh2_create_NC_file_v2(dout, DT):
   from netCDF4 import Dataset
   
   f1 = 'ncas-rga3-1' #instrument name
   ...
   f4 = 'co-h2-concentration' #data product
...
def coh2_NC_Global_Attributes_v2(fn_nc, meta, ET):
...
def coh2_NC_Dimensions_v2(fn_nc, ET):
...
def coh2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon):
   #time
   ...
   #H2 conc
   H2 = fn_nc.createVariable('mole_fraction_of_molecular_hydrogen_in_air', np.float32, ('time',),fill_value=-1.00e+20)
   #variable attributes
   H2.type = 'float32'
   H2.units = '1e-9'
   H2.practical_units = 'ppb'
   H2.long_name = 'Mole Fraction of molecular Hydrogen in air'
   XX = data.h2
   np.putmask(XX, data.flag_h2 != 1, np.nan)
   H2.valid_min = np.float32(np.nanmin(XX))
   H2.valid_max = np.float32(np.nanmax(XX))
   H2.cell_methods = 'time: point'
   H2.coordinates = 'latitude longitude'
   H2.chemical_species = 'H2'
   #write data
   H2[:] = np.float32(data.h2)
   
   #Qc flag H2
   qc_flags_h2 = fn_nc.createVariable('qc_flag_h2', np.int8, ('time',))
   #variable attribute
   qc_flags_h2.type = 'byte'
   qc_flags_h2.units = '1'
   qc_flags_h2.long_name = 'Data Quality Flag: H2'
   qc_flags_h2.flag_values = '0b,1b,2b,3b'
   qc_flags_h2.flag_meanings = 'not_used' + '\n'
   qc_flags_h2.flag_meanings = qc_flags_h2.flag_meanings + 'good_data' + '\n'
   qc_flags_h2.flag_meanings = qc_flags_h2.flag_meanings + 'suspect_data_data_not_quality_controlled' + '\n'
   qc_flags_h2.flag_meanings = qc_flags_h2.flag_meanings + 'bad_data_do_not_use' 
   #write data
   qc_flags_h2[:] = np.int8(data.flag_h2)
   
def NC_coh2_v2(pd, np, dout, meta, data):
   fn_nc = coh2_create_NC_file_v2(dout, data.DT)
   
   lat, lon = coh2_NC_Global_Attributes_v2(fn_nc, meta, data.ET)
   coh2_NC_Dimensions_v2(fn_nc, data.ET)
   coh2_NC_VaraiblesAndData_v2(fn_nc, data, np, lat, lon)
