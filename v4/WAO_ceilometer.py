...
def parse_block_v2(lines, start_of_block):
...
def parse_line1_v2(lines, start_of_block, np, data):
   ...
   data.L11 = np.array(l11)
   data.L12 = np.array(l12)
   data.L13 = np.array(l13)
   
   return data
   
def parse_CBH_v2(lines, start_of_block, np, data, offset):
   l21 = []
   l22 = []
   cbh1 = []
   cbh2 = []
...
def parse_HK_v2(lines, start_of_block, np, data, offset):
   l31 = []
   l32 = []
   l33 = []
   l34 = []
   ...
   data.L31 = np.array(np.float32(l31))
   data.L32 = np.array(l32)
   data.L33 = np.array(np.float32(l33))
   data.L34 = np.array(np.float32(l34) + 273.15)
   data.L35 = np.array(np.float32(l35))
   data.L36 = np.array(np.float32(l36))
   data.L37 = np.array(np.float32(l37))
   data.L38 = np.array(np.float32(l38))
   data.L39 = np.array(l39)
   data.L310 = np.array(np.float32(l310)/10000) #now in sr-1
        
   return data 

def parse_PROF_v2(lines, start_of_block, np, data, offset):
   line4 = []
...
def ceil_parse_v2(din, data): 
   ...
   start_of_block = parse_block_v2(lines, start_of_block)    
    
   #parse time
   data = parse_time_v2(lines, start_of_block, np, data) 
   
   #parse line 1 - housekeeping
   data = parse_line1_v2(lines, start_of_block, np, data) # housekeeping 1
   
   #parse line 2 - cloud base height
   data = parse_CBH_v2(lines, start_of_block, np, data, 1) # cloudbase height
  
   #parse line 3 - housekeeping  
   data = parse_HK_v2(lines, start_of_block, np, data, 2) # housekeeping 2

   #parse line 4 - profile   
   data = parse_PROF_v2(lines, start_of_block, np, data, 3) # backscatter profile 
